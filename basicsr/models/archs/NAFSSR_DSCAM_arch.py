...
########################
        ...
        # print(k_windows.shape)#torch.Size([1, 18432, 16])
        k_windows_r = rearrange(k_windows_r, 'b (nc ch owh oww) nw -> nc (b nw) (owh oww) ch', nc=1, ch=c_r, owh=self.overlap_win_size, oww=self.overlap_win_size).contiguous() # 1, nw*b, ow*ow, c

        v_windows_r = self.unfold(x_l)
        v_windows_r = rearrange(v_windows_r, 'b (nc ch owh oww) nw -> nc (b nw) (owh oww) ch', nc=1, ch=c_r, owh=self.overlap_win_size, oww=self.overlap_win_size).contiguous() # 1, nw*b, ow*ow, c
...
class NAFSSR_DSCAM(Local_Base, NAFNetSR):
