...
class SCAM(nn.Module):
    ...
    def forward(self, feat, x_size):
        ...
        conv_x_l = self.conv_block(x_l)
        conv_x_l = conv_x_l.permute(0, 2, 3, 1).contiguous().view(b, h , w, c)

        conv_x_r = self.conv_block(x_r)
        conv_x_r = conv_x_r.permute(0, 2, 1, 3).contiguous().view(b, h , c, w)
...
##########################################################
        ...
        Q_l = self.l_proj1(self.norm_l(x_l_.permute(0, 3, 2, 1)) + conv_x_l * self.conv_scale).permute(0, 2, 3, 1)  # B, H, W, c
        Q_r_T = self.r_proj1(self.norm_r(x_r_.permute(0, 3, 2, 1)) + conv_x_r * self.conv_scale).permute(0, 2, 1, 3) # B, H, c, W (transposed)
...
class ST_DSCAM_CA(Local_Base, STNetSR):
    def __init__(self,
                 img_size=64,
                 patch_size=1,
                 in_chans=3,
