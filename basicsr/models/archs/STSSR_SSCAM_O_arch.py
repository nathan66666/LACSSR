...
class SCAM(nn.Module):
    ...
    def __init__(self, dim,
        ...
        self.overlap_ratio_h = overlap_ratio_h
        self.overlap_ratio_w = overlap_ratio_w
        self.overlap_win_size_h = int(window_size * overlap_ratio_h) + window_size
        self.overlap_win_size_w = int(window_size * overlap_ratio_w) + window_size

        self.qkv = nn.Linear(dim, dim * 3,  bias=qkv_bias)
        self.unfold = nn.Unfold(kernel_size=(self.overlap_win_size_h, self.overlap_win_size_w), stride=window_size, padding=((self.overlap_win_size_h-window_size)//2, (self.overlap_win_size_w-window_size)//2))

        # define a parameter table of relative position bias
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((window_size + self.overlap_win_size_h - 1) * (window_size + self.overlap_win_size_w - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH
    ...
    def calculate_rpi_oca(self):
        ...
        window_size_h_ext = self.window_size + int(self.overlap_ratio_h * self.window_size)
        window_size_w_ext = self.window_size + int(self.overlap_ratio_w * self.window_size)
        ...
        coords_ext = torch.stack(torch.meshgrid([coords_h, coords_w]))  # 2, ws, wse
        coords_ext_flatten = torch.flatten(coords_ext, 1)  # 2, ws*wse

        relative_coords = coords_ext_flatten[:, None, :] - coords_ori_flatten[:, :, None]   # 2, ws*ws, ws*wse

        relative_coords = relative_coords.permute(1, 2, 0).contiguous()  # ws*ws, ws*wse, 2
        relative_coords[:, :, 0] += window_size_ori - window_size_h_ext + 1 # shift to start from 0
        relative_coords[:, :, 1] += window_size_ori - window_size_w_ext + 1

        relative_coords[:, :, 0] *= window_size_ori + window_size_h_ext - 1
    ...
    def forward(self, feat, x_size):
        ...
        k_windows = rearrange(k_windows, 'b (nc ch owh oww) nw -> nc (b nw) (owh oww) ch', nc=1, ch=c, owh=self.overlap_win_size_h, oww=self.overlap_win_size_w).contiguous() # 1, nw*b, ow*ow, c

        v_windows = self.unfold(x_r)
        v_windows = rearrange(v_windows, 'b (nc ch owh oww) nw -> nc (b nw) (owh oww) ch', nc=1, ch=c, owh=self.overlap_win_size_h, oww=self.overlap_win_size_w).contiguous() # 1, nw*b, ow*ow, c
        ...
        relative_position_bias = self.relative_position_bias_table[rpi.view(-1)].view(
            self.window_size * self.window_size, self.overlap_win_size_h * self.overlap_win_size_w, -1)  # ws*ws, wse*wse, nH
...
##########################################################
        ...
        k_windows_ = rearrange(k_windows_, 'b (nc ch owh oww) nw -> nc (b nw) (owh oww) ch', nc=1, ch=c, owh=self.overlap_win_size_h, oww=self.overlap_win_size_w).contiguous() # 1, nw*b, ow*ow, c

        v_windows_ = self.unfold(x_l.permute(0, 3, 1, 2))
        v_windows_ = rearrange(v_windows_, 'b (nc ch owh oww) nw -> nc (b nw) (owh oww) ch', nc=1, ch=c, owh=self.overlap_win_size_h, oww=self.overlap_win_size_w).contiguous() # 1, nw*b, ow*ow, c
        ...
        Q_r = self.r_proj1(self.norm_r(x_r_.permute(0, 3, 2, 1))).permute(0, 2, 3, 1)  # B, H, c, W (transposed)
        ...
        # print(v_l.shape, F_r2l.shape, (Q_l.permute(0, 3, 1, 2)).shape)
        x_l = v_l + Q_l.permute(0, 3, 1, 2)
        x_r = v_r + Q_r.permute(0, 3, 1, 2)
...
class STSSR_SSCAM_o(Local_Base, STNetSR):
    def __init__(self,
                 img_size=64,
                 patch_size=1,
                 in_chans=3,
        ...
        STNetSR.__init__(self,
                         ...
                         mlp_ratio=mlp_ratio,
                         qkv_bias=qkv_bias,
                         overlap_ratio_h = overlap_ratio_h,
                         overlap_ratio_w = overlap_ratio_w,
                         drop_rate=drop_rate,
