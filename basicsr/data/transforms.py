...
def paired_random_crop_hw(img_gts, img_lqs, gt_patch_size_h, gt_patch_size_w, scale, gt_path):
    ...
    h_lq, w_lq, _ = img_lqs[0].shape
    #print(len(img_lqs),img_lqs[0].shape,h_lq,w_lq)
    h_gt, w_gt, _ = img_gts[0].shape
...
def gt_random_crop_hw(img_gts, gt_patch_size_h, gt_patch_size_w, scale, gt_path):

    if not isinstance(img_gts, list):
        img_gts = [img_gts]
    h_gt, w_gt, _ = img_gts[0].shape
    #print(img_gts[0].shape)
    top_gt = random.randint(0, h_gt - gt_patch_size_h)
    left_gt = random.randint(0, w_gt - gt_patch_size_w)
